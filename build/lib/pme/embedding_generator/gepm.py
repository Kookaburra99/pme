...
class GEPM(nn.Module):
    """
    Implementation of GEPM (Graph Embeddings for Process Mining) model
    """

    def __init__(self, num_categories: int, emb_size: int,
                 device: torch.device, seed: int = 21):
        """
        Creates the model for GEPM embeddings
    ...
    def get_random_walks(self, graph: nx.DiGraph, split: str,
                         n: int = 100, l: int = 10):
        """
        Generate random walks from each node based on the weights
        of the edges
        :param graph: Networkx DiGraph of the process, with weights
        :param split: Partition to use (train, val or test)
        :param n: Number of random walk starting in each node
        :param l: Length of the random walks
        """
        random_walks = []
                    ...
                    if current_node == -1:  # Padding
                        ...
                        if neighbors:
                            ...
                            for neig in neighbors:
                                weights.append(graph[current_node][neig]['weight'])
                            next = np.random.choice(neighbors, 1, p=weights).item()
        ...
        if split == 'train':
            self.train_walks = random_walks
        if split == 'val':
            self.val_walks = random_walks
        if split == 'test':
            self.test_walks = random_walks

    def get_pairs_dataloader(self, win_size: int,
                             batch_size: int, split: str):
        """
        Get each feature from every walk and their context represented
        in one vector
        ...
        walks = []
        if split == 'train':
            walks = self.train_walks
        if split == 'val':
            walks = self.val_walks
        if split == 'test':
            walks = self.test_walks
    ...
    def train_embeddings(self, learning_rate: float, epochs: int, early_stop: int = None,
                         path_model: str = './models/GEPM_model.m'):
        """
        Train the GEPM model with the stored 'train_loader'
        and validate it with the stored 'val_loader'. The best
        model in validation is kept
        :param learning_rate: The initial learning rate
        :param epochs: Number of epochs of training
        :param path_model: Full path where model will be stored.
        Default is './models/GEPM_model.m'
...
def get_gepm_embeddings(graph: nx.DiGraph, win_size: int, emb_size: int,
    ...
    start_time = time()
    gepm_model = GEPM(num_categories, emb_size, device, seed)
    gepm_model.get_random_walks(graph, 'train', 100, 10)
    gepm_model.get_random_walks(graph, 'val', 100, 10)
    gepm_model.get_pairs_dataloader(win_size, batch_size, 'train')
    gepm_model.get_pairs_dataloader(win_size, batch_size, 'val')
    gepm_model.train_embeddings(learning_rate, epochs, 10)
    end_time = time()

    dict_embeddings = {}
    weights = list(gepm_model.parameters())[0].data
    for i in range(gepm_model.num_categories):
